...
public class PrientGoods extends Thread implements Printable{  
	 //O2OMainMenu mm = null;
//	private Properties prop1 = new Properties();
    static DecimalFormat df = new DecimalFormat("0.00");
    static String totalMoneys = null;
    static String changeMoneys = null;
    static int row =0;
    static double tax = 0.00;
    static double paymoney = 0.00;
    static String bankMoney = "0.00";
    static String cashMoney = null;
    static String sn ="";
	...
	static double discountAmount = 0.00; 
...
//		}
    	...
    	String lsh = PropertiesUtil.getPosNum()+0+0+count;
       ...
       // g2d.drawString("单价", 55, 80);
        ...
        for(i=0;i<row;i++){
        	String goodsNames = table.getValueAt(i, 2)+"";
        	String goodsName = "";
        	String goodsName1 = "";
        	...
        	if(goodsLength<17){
    		 ...
    		 g2d.drawString((String) table.getValueAt(i, 1),Prient.transX,200+i*30);
             g2d.drawString(table.getValueAt(i, 8)+"*"+table.getValueAt(i, 7), 38+Prient.transX, 200+i*30);
             ...
             g2d.drawString(table.getValueAt(i, 9)+"", 95+Prient.transX, 200+i*30);
        }
        g2d.setFont(new Font("Default", Font.PLAIN, 8));
        g2d.drawString("-------------------------------------------",0+Prient.transX, 210+i*30);
        g2d.drawString("购买件数: " + totalNumber, 0+Prient.transX, 220+i*30);  
        g2d.drawString("应付: " + totalMoney, 0+Prient.transX, 230+i*30);
        g2d.drawString("税金: " + taxs, 0+Prient.transX, 240+i*30);
        g2d.drawString("刷卡金额:" + bankMoney , 0+Prient.transX, 250+i*30);
        g2d.drawString("实收金额:" + String.format("%.2f", amount), 0+Prient.transX, 260+i*30);
        g2d.drawString("优惠合计:" + String.format("%.2f", discountAmount) , 0+Prient.transX, 270+i*30);
        g2d.drawString("现金金额:"+ cashMoney, 0+Prient.transX, 280+i*30);
        g2d.drawString("找零:" + changeMoneys , 0+Prient.transX, 290+i*30);
        g2d.drawString("-------------------------------------------", 0+Prient.transX, 300+i*30);  
        //g2d.drawString("时间: " + aString , 0, 120+i*20);
        g2d.drawString("欢迎您再次光临！", 0+Prient.transX, 330+i*30);
        g2d.drawString("-------------------------------------------",0+Prient.transX, 320+i*30);
...
//        return PAGE_EXISTS;  
    ...
    public void run() { 
        ...
        book.append(new PrientGoods(), pf);  
        ...
        try {  
         ...
         // 清空table数据
            ...
            // 清空table数据
            O2OMainMenu.memberNumber.setText("");
            DefaultTableModel model = (DefaultTableModel) O2OMainMenu.table.getModel();
            while (model.getRowCount() > 0) {
                model.removeRow(model.getRowCount() - 1);
            }
        }
